...
void DFPN::signalHandler([[maybe_unused]] int sig) {
    ...
    **/
    g_timer.stop();
    std::cout << "? None " << g_timer.duration().count() << " " << g_counter << std::endl;
    exit(EXIT_FAILURE);
}
...
void DFPN::MID(Node& node) {  // NOLINT
    ...
    // the node must have at least one child at this point; otherwise it's a terminal
    ...
    if (best_child_index != UINT16_MAX) {
        ...
        best_move = node.children[0].move;  // if not being updated, then it has only one child
    }
    */
    saveProofAndDisproofNumbers(node, node.phi, node.delta);
}
...
void DFPN::saveProofAndDisproofNumbers(Node& node, uint32_t phi, uint32_t delta) {
    tt[node.id] = std::make_pair(phi, delta);
}

void DFPN::retrieveProofAndDisproofNumbers(Node& node, uint32_t& phi, uint32_t& delta) {
