...
void Game::floodFill(const Pos& starting_pos, PosSet& filled_visited, PosSet& empty_visited) const {  // NOLINT
...
bool Game::isValid() const {
    ...
    for (auto pos : getFilledPositions()) {
        ...
        // For each of the ones we already discovered in the flood fill, remember we were there already
        // We don't need to examine them again
...
void Game::reset(const Pos& pos) {
    data[pos.row * width + pos.col] = 0;
}
