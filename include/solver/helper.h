...
struct Pos {
    ...
    inline bool operator==(const Pos& other) const { return row == other.row && col == other.col; }

    [[nodiscard]] std::string toString() const {
        return std::to_string((unsigned int)col) + " " + std::to_string((unsigned int)row);
    }

    struct Hash {
        std::size_t operator()(const Pos& pos) const { return std::hash<uint8_t>()((pos.row << 4) | pos.col); }
    };
};
...
struct Move {
    ...
    [[nodiscard]] std::string toString() const { return pos.toString() + " " + std::to_string((unsigned int)value); }
};
